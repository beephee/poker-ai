...
outfile = 'Monte Carlo Cards Estimation_5.txt'
...
for total_cards in [5,]:
    ...
    for round_game in range(NUM_TRIES):
        ...
        win_prob = []
        for j in range(20):
            win_prob.append(estimate_hole_card_win_rate(100, 2, gen_cards(hole_card), gen_cards(comm_card)) * 100)
        avg_100 = sum(win_prob)/len(win_prob)
        win_prob_1k = []
        for j in range(20):
            win_prob_1k.append(estimate_hole_card_win_rate(1000, 2, gen_cards(hole_card), gen_cards(comm_card)) * 100)
        avg_1000 = sum(win_prob_1k)/len(win_prob_1k)
        deviations.append(math.fabs(avg_1000 - avg_100))
        ctr += 1
        print('run ' + str(ctr) + ' of ' + str(NUM_TRIES) + ' for num_cards = ' + str(total_cards) + ' done')

    best_dev = sum(deviations)/len(deviations)
    with open(outfile,'a') as writefile:
        writefile.write('\n FOR ' + str(total_cards) + ' CARDS\n')
        writefile.write('Best total number is ' + str(best_dev) + '\n')
        writefile.write('Max is ' + str(max(deviations)) + '\n')
        writefile.write('Min is ' + str(min(deviations)) + '\n')
        writefile.write('Range is ' + str(max(deviations) - min(deviations)) + '\n')
    print('\n FOR ' + str(total_cards) + ' CARDS\n')
    print('Best total number is ' + str(best_dev))
    print('Max is ' + str(max(deviations)))
    print('Min is ' + str(min(deviations)))
    print('Range is ' + str(max(deviations) - min(deviations)))
