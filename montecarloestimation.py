...
outfile = 'Monte Carlo Estimation.txt'
poss_cards = list(str(i) for i in range(2,10)) + ['T','J','Q','K','A']
poss_suits = ['D','C','H','S']
all_cards = [''.join(list(p)) for p in itertools.product(poss_suits, poss_cards)]
best_nb = []
ctr = 0

for round_game in range(NUM_TRIES):

    curr_cards = all_cards
    hole_card = []
    comm_card = []

    hole_card.append(random.choice(all_cards))
    new = random.choice(all_cards)
    while new in hole_card:
        new = random.choice(all_cards)
    hole_card.append(new)

    with open(outfile,'a') as writefile:
        win_probs = []
        x_vals = [i*1000 for i in range(1,101)]
        for i in x_vals:
            win_prob = []
            for j in range(20):
                win_prob.append(estimate_hole_card_win_rate(i, 2, gen_cards(hole_card), gen_cards(comm_card)) * 100)
            avg = sum(win_prob)/len(win_prob)
            if hole_card[0][0] == hole_card[1][0]:              # same suit
                try:
                    actual = preflop_samesuit[hole_card[0][1]+hole_card[1][1]]
                except KeyError:
                    actual = preflop_samesuit[hole_card[1][1]+hole_card[0][1]]
            else:
                try:
                    actual = preflop_offsuit[hole_card[0][1]+hole_card[1][1]]
                except KeyError:
                    actual = preflop_offsuit[hole_card[1][1]+hole_card[0][1]]
            diff = math.fabs(actual-avg)
            if diff <= THRESHOLD:
                writefile.write('Best number of simulations  for ' +  ','.join(hole_card) + ' is ' + str(i) + ' with deviation of ' + str(diff) + '%\n')
                #best_nb.append(i)
                best_nb.append(diff)
                ctr += 1
                print('run ' + str(ctr) + ' of ' + str(NUM_TRIES) + ' done')
                break


best_sim_nb = sum(best_nb)/len(best_nb)
with open(outfile,'a') as writefile:
    writefile.write('Best total number is ' + str(best_sim_nb) + '\n')
    writefile.write('Max is ' + str(max(best_nb)) + '\n')
    writefile.write('Min is ' + str(min(best_nb)) + '\n')
    writefile.write('Range is ' + str(max(best_nb) - min(best_nb)) + '\n')
print('Best total number is ' + str(best_sim_nb))
print('Max is ' + str(max(best_nb)))
print('Min is ' + str(min(best_nb)))
print('Range is ' + str(max(best_nb) - min(best_nb)))
